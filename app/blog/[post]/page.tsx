...
const fallbackImage: string =
  "https://res.cloudinary.com/kunjnakrani/image/upload/v1692636087/kunjnakrani/blog.png";

// Dynamic metadata for SEO
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const slug = params.post;
  let post: PostType | undefined = localPosts.find((p) => p.slug === slug);
  ...
  return {
    title: `${post.title}`,
    metadataBase: new URL(`https://kunjnakrani.com/blog/${post.slug}`),
    description: post.description,
    publisher: post.author.name,
    keywords: post.tags,
    alternates: {
      canonical:
        post.canonicalLink || `https://kunjnakrani.com/blog/${post.slug}`,
    },
    openGraph: {
      images:
        urlFor(post.coverImage?.image).width(1200).height(630).url() ||
        fallbackImage,
      url: `https://kunjnakrani.com/blog/${post.slug}`,
      title: post.title,
      description: post.description,
      type: "article",
