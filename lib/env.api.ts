...
function checkValue<T>(
  ...
  if (value === undefined) {
    // Return dummy value to prevent crash, since we are using local data
    console.warn(`Missing Environment Variable: ${errorMsg}. Using placeholder.`);
    return "placeholder-value" as unknown as T;
  }
  return value;
}
